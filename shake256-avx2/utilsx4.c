    ...
    void SPX_treehashx4_##name(                                                                    \
        ...
        for (idx = 0; idx < (uint32_t)(1 << (tree_height)); idx++) {                               \
            ...
            for (j = 0; j < 4; j++) {                                                              \
                if ((leaf_idx[j] ^ 0x1) == idx) {                                                  \
                    memcpy(auth_pathx4 + j * (tree_height)*SPX_N,                                  \
                           stackx4 + j * ((tree_height) + 1) * SPX_N + (offset - 1) * SPX_N,       \
                           SPX_N);                                                                 \
                }                                                                                  \
            }                                                                                      \
                                                                                                   \
            ...
            while (offset >= 2 && heights[offset - 1] == heights[offset - 2]) {                    \
                ...
                for (j = 0; j < 4; j++) {                                                          \
                    SPX_set_tree_height(tree_addrx4 + j * 8, heights[offset - 1] + 1);             \
                    SPX_set_tree_index(tree_addrx4 + j * 8,                                        \
                                       tree_idx + (idx_offset[j] >> (heights[offset - 1] + 1)));   \
                }                                                                                  \
                /* Hash the top-most nodes from the stack together. */                             \
                SPX_thashx4_2(stackx4 + 0 * ((tree_height) + 1) * SPX_N + (offset - 2) * SPX_N,    \
...
treehashx4_variant(FORS_HEIGHT, SPX_FORS_HEIGHT)
