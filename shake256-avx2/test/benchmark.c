...
int main()
{
    ...
    MEASURE("  - WOTS pk gen 4x.. ", (1 << SPX_TREE_HEIGHT) / 4, wots_gen_pkx4(wots_pk, sk, pk, (uint32_t *) addr));
    ...
    MEASURE("  - WOTS pk gen x4.. ", SPX_D * (1 << SPX_TREE_HEIGHT) / 4, wots_gen_pkx4(wots_pk, sk, pk, (uint32_t *) addr));
