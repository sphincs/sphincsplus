...
int SPX_crypto_sign_seed_keypair(
    ...
    SPX_initialize_hash_function(&hash_state_seeded, pk, sk);

    /* Compute root node of the top-most subtree. */
    SPX_treehash_TREE_HEIGHT(
...
int SPX_crypto_sign_keypair(
    ...
    union {
        __m128 _x[SPX_CRYPTO_PUBLICKEYBYTES/16];
        uint8_t pk[SPX_CRYPTO_PUBLICKEYBYTES];
    } aligned_pk;

    // guarantee alignment of sk
    union {
        __m128 _x[SPX_CRYPTO_SECRETKEYBYTES/16];
        uint8_t sk[SPX_CRYPTO_SECRETKEYBYTES];
    } aligned_sk;

    union {
        __m128 _x[SPX_CRYPTO_SEEDBYTES/16];
        uint8_t seed[SPX_CRYPTO_SEEDBYTES];
    } aligned_seed;
    randombytes(aligned_seed.seed, SPX_CRYPTO_SEEDBYTES);

    SPX_crypto_sign_seed_keypair(
        aligned_pk.pk, aligned_sk.sk, aligned_seed.seed);
    memcpy(pk, aligned_pk.pk, SPX_CRYPTO_PUBLICKEYBYTES);
    memcpy(sk, aligned_sk.sk, SPX_CRYPTO_SECRETKEYBYTES);
...
int SPX_crypto_sign_signature(
    ...
    memcpy(aligned_sk.sk, sk, SPX_CRYPTO_SECRETKEYBYTES);
    sk = aligned_sk.sk;
    ...
    for (i = 0; i < SPX_D; i++) {
        SPX_set_layer_addr(tree_addr, i);
    ...
    memcpy(orig_sig, aligned_sig.sig, SPX_BYTES);
    *siglen = SPX_BYTES;
...
int SPX_crypto_sign_verify(
    ...
    memcpy(aligned_pk.pk, pk, SPX_CRYPTO_PUBLICKEYBYTES);
    pk = aligned_pk.pk;
