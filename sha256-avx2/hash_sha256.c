...
void SPX_initialize_hash_function(
    ...
    SPX_seed_state(&hash_state_seeded->x1, pub_seed);
    SPX_seed_statex8(&hash_state_seeded->x8, pub_seed);
 ...
 */
void SPX_prf_addr(unsigned char *out, const unsigned char *key, const uint32_t addr[8],
...
#define SPX_INBLOCKS (((SPX_N + SPX_PK_BYTES + SPX_SHA256_BLOCK_BYTES - 1) & \
    ...
    SPX_mgf1(bufp, SPX_DGST_BYTES, seed, SPX_SHA256_OUTPUT_BYTES);
