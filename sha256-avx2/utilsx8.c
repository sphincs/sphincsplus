                ...
                const hash_state *state_seeded)
{
    ...
    for (idx = 0; idx < (uint32_t)(1 << tree_height); idx++) {
        ...
        while (offset >= 2 && heights[offset - 1] == heights[offset - 2]) {
            ...
            for (j = 0; j < 8; j++) {
                SPX_set_tree_height(tree_addrx8 + j*8, heights[offset - 1] + 1);
                SPX_set_tree_index(tree_addrx8 + j*8,
                                   tree_idx + (idx_offset[j] >> (heights[offset-1] + 1)));
            }
            /* Hash the top-most nodes from the stack together. */
            SPX_thashx8_2(stackx8 + 0*(tree_height + 1)*SPX_N + (offset - 2)*SPX_N,
...
#define treehashx8_variant(name, size)                                                              \
    ...
    {                                                                                               \
        ...
        unsigned char stackx8[8*((size) + 1)*SPX_N];                                                \
...
treehashx8_variant(FORS_HEIGHT, SPX_FORS_HEIGHT)
