             ...
             const unsigned char *pub_seed, uint32_t addrx8[8*8])
{
    ...
    for (i = 0; i < 8; i++) {
        memcpy(bufx8 + i*(SPX_N + SPX_ADDR_BYTES + inblocks*SPX_N), pub_seed, SPX_N);
        addr_to_bytes(bufx8 + SPX_N + i*(SPX_N + SPX_ADDR_BYTES + inblocks*SPX_N), addrx8 + i*8);
    }

    memcpy(bufx8 + SPX_N + SPX_ADDR_BYTES +
        0*(SPX_N + SPX_ADDR_BYTES + inblocks*SPX_N), in0, inblocks * SPX_N);
    memcpy(bufx8 + SPX_N + SPX_ADDR_BYTES +
        1*(SPX_N + SPX_ADDR_BYTES + inblocks*SPX_N), in1, inblocks * SPX_N);
    memcpy(bufx8 + SPX_N + SPX_ADDR_BYTES +
        2*(SPX_N + SPX_ADDR_BYTES + inblocks*SPX_N), in2, inblocks * SPX_N);
    memcpy(bufx8 + SPX_N + SPX_ADDR_BYTES +
        3*(SPX_N + SPX_ADDR_BYTES + inblocks*SPX_N), in3, inblocks * SPX_N);
    memcpy(bufx8 + SPX_N + SPX_ADDR_BYTES +
        4*(SPX_N + SPX_ADDR_BYTES + inblocks*SPX_N), in4, inblocks * SPX_N);
    memcpy(bufx8 + SPX_N + SPX_ADDR_BYTES +
        5*(SPX_N + SPX_ADDR_BYTES + inblocks*SPX_N), in5, inblocks * SPX_N);
    memcpy(bufx8 + SPX_N + SPX_ADDR_BYTES +
        6*(SPX_N + SPX_ADDR_BYTES + inblocks*SPX_N), in6, inblocks * SPX_N);
    memcpy(bufx8 + SPX_N + SPX_ADDR_BYTES +
        7*(SPX_N + SPX_ADDR_BYTES + inblocks*SPX_N), in7, inblocks * SPX_N);
