                     ...
                     unsigned long long seed_length)
{
    ...
    if (sk_seed != NULL) {
        SPX_haraka_S(buf, 40*16, sk_seed, seed_length, state);
    ...
    SPX_haraka_S(buf, 40*16, pk_seed, seed_length, state);
                              ...
                              const harakactx * state)
{
    ...
    while (mlen >= HARAKAS_RATE) {
        ...
        SPX_haraka512_perm_x4(s, s, state);
        mlen -= HARAKAS_RATE;
        m0 += HARAKAS_RATE;
        m1 += HARAKAS_RATE;
        m2 += HARAKAS_RATE;
...
void SPX_haraka_S_inc_absorb(uint8_t *s_inc, const uint8_t *m, size_t mlen, const harakactx* state)
{
    ...
    while (mlen + s_inc[64] >= HARAKAS_RATE) {
        ...
        SPX_haraka512_perm(s_inc, s_inc, state);
    }
...
void SPX_haraka_S(unsigned char *out, unsigned long long outlen,
...
void SPX_haraka_Sx4(unsigned char *out0,
                unsigned char *out1,
                unsigned char *out2,
                unsigned char *out3,
