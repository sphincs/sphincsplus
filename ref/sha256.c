 ...
 */
void SPX_compress_address(unsigned char *out, const uint32_t addr[8]) {
    SPX_ull_to_bytes(out,      1, addr[0]); /* drop 3 bytes of the layer field */
    SPX_ull_to_bytes(out + 1,  4, addr[2]); /* drop the highest tree address word */
    SPX_ull_to_bytes(out + 5,  4, addr[3]);
    SPX_ull_to_bytes(out + 9,  1, addr[4]); /* drop 3 bytes of the type field */
    SPX_ull_to_bytes(out + 10, 4, addr[5]);
    SPX_ull_to_bytes(out + 14, 4, addr[6]);
    SPX_ull_to_bytes(out + 18, 4, addr[7]);
}
...
void SPX_mgf1(
    ...
    for (i = 0; (i + 1)*SPX_SHA256_OUTPUT_BYTES <= outlen; i++) {
        SPX_ull_to_bytes(input_plus_four_bytes + inlen, 4, i);
 ...
 **/
void SPX_seed_state(sha256ctx *hash_state_seeded, const unsigned char *pub_seed) {
